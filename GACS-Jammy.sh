...
# Copy GenieACS to node_modules
echo -e "${YELLOW}Copying GenieACS to /usr/lib/node_modules/...${NC}"
if [ -d "genieacs" ]; then
    cp -r genieacs /usr/lib/node_modules/
    chown -R root:root /usr/lib/node_modules/genieacs
    chmod -R 755 /usr/lib/node_modules/genieacs
    echo -e "✅ ${GREEN}GenieACS copied successfully${NC}"
else
    echo -e "❌ ${RED}GenieACS directory not found${NC}"
    exit 1
fi
...
if [ -d "db" ]; then
    # Stop GenieACS services before restore
    ...
    if mongosh --eval 'db.runCommand({ connectionStatus: 1 })' --quiet >/dev/null 2>&1; then
        ...
        if mongorestore --db genieacs --drop db; then
        ...
        echo -e "${YELLOW}Trying to start MongoDB...${NC}"
        systemctl start mongod
        sleep 5
        
        if mongorestore --db genieacs --drop db; then
            echo -e "✅ ${GREEN}Database restored successfully on second attempt${NC}"
        else
            echo -e "❌ ${RED}Failed to restore database. Please check MongoDB logs.${NC}"
            systemctl status mongod --no-pager
        fi
    fi
else
    echo -e "❌ ${RED}Database directory 'db' not found${NC}"
    echo -e "${YELLOW}Skipping database restore${NC}"
fi

# Restart GenieACS services
echo -e "\n${YELLOW}Restarting GenieACS services...${NC}"
systemctl daemon-reload
for service in cwmp fs ui nbi; do
    echo -n "Restarting genieacs-$service... "
    systemctl restart genieacs-$service
    sleep 2
    if systemctl is-active --quiet genieacs-$service; then
        echo -e "${GREEN}OK${NC}"
    else
        echo -e "${RED}FAILED${NC}"
        systemctl status genieacs-$service --no-pager
    fi
done
